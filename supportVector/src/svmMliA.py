...
def testRbf(k1 = 0.5):
    ...
    Returns:
        无
    """
    dataArr,labelArr = loadDataSet(r'F:\Machine-Learning-master\Machine-Learning-master\SVM\testSetRBF.txt')                        #加载训练集
    ...
    dataArr,labelArr = loadDataSet(r'F:\Machine-Learning-master\Machine-Learning-master\SVM\testSetRBF2.txt')                         #加载测试集
    errorCount = 0
    datMat = np.mat(dataArr); labelMat = np.mat(labelArr).transpose()         
    m,n = np.shape(datMat)
    for i in range(m):
